class Team < ApplicationRecord
  ...
  scope :season_roster_spots, -> (season_id:, preload: false) {
    ...
    includes({roster_spots: user_inc}).where(roster_spots: {season_id: season_id}).order('roster_spots.team_id')
  }
  ...
  def sorted_members
    ...
    comparator = lambda do |rs|
      ...
      return Rails.application.config.team_role_ranks.length.to_s + rs.user.last_name
    end
